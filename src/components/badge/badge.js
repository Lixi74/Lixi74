...
function Badge({ splashScreen }) {
    ...
    const usernameContainerClass = `username-container ${splashScreen ? 'username-splash' : 'username-header'} ${themeClass}`;

    return (
      <Component {...componentProps} style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg
          width={svgWidth}
          height={svgHeight}
          viewBox={`0 0 400 100`}
        ...
        >
          ...
          <foreignObject x="90" y="10" width="300" height="100">
            <div
              className={usernameContainerClass}
            >
              {greeting.username}
            </div>
          </foreignObject>
